kintone.events.on(['mobile.app.record.edit.change.Status_Radio', 'mobile.app.record.create.change.Status_Radio','app.record.index.edit.change.Status_Radio'], function(event) {
    var record = event.record;
    var trigger_status = ['Give Up','Pending','Won','Lost'];
    if (trigger_status.includes(record.Status_Radio.value)) {
        console.log('Active');
        var localDate = new Date(record.Updated_datetime.value);
